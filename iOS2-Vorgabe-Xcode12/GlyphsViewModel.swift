...
class GlyphsViewModel: ObservableObject {
    private var glyphs: [Glyph] = [Circle(), Square(), Diamond()] // the model: add your glyphs to the array
    ...
    func glyph(index: Int, changedType type: GlyphType) {
        ...
        //als old glyph speichern -> weniger Speicher
        let oldG = glyphs[index]
        switch (type) {
            case.circle:
                glyphs[index] = Circle()
            case.diamond:
        ...
        glyphs[index].color = oldG.color
        glyphs[index].size = oldG.size
        glyphs[index].showDescription = oldG.showDescription
        glyphs[index].center.x = oldG.center.x
        glyphs[index].center.y = oldG.center.y
    }
